    ...
    return func(path string, f os.FileInfo, err error) error {
        ...
        for _, r := range regexes {
            if r.MatchString(path) {
				...
				var numOfAddedFiles int
                if w != nil && (numOfAddedFiles)>0 {
                    ...
                    w.Write([]byte(`"`+(strconv.FormatInt(int64(numOfAddedFiles), 10))+`":  `))
...
func MainPage(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(w, "<html><body><H1>This API acts as a web scraper that indexes files</H1></body>")
}
...
func IndexFiles(w http.ResponseWriter, r *http.Request) {
	...
	if regexOK{
		...
		filepath.Walk(`/home/cabox `+location[0], walkFn2(w, regEx[0]))
	}else {
