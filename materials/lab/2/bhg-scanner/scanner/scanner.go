...
// Useage: Navigate the command line to the scanner folder:
//		   go test - Lists all open and closed ports (Expects ports 80 and 22 to be open)
// If list and time ran is what you're looking for:
//		Navigate the command line to the main folder, go build main.go, time ./main
//		
// {TODO 1: FILL IN}
...
func worker(ports, results chan int, totalPorts int) {
	for p := range ports {
...
func PortScanner(totalPorts int) (int, int) {
	totalPorts = 1024
	...
	for i := 0; i < cap(ports); i++ {
		go worker(ports, results, totalPorts)
	}
